...
这是一个围绕 **桌面 /goal 工作流、Project Memory、项目化会话侧边栏、插件热刷新和桌面稳定性** 的版本。

相比 `v0.2.6`，本次把桌面端从“能看到更多能力”继续推进到“能持续使用这些能力”：`/goal` 现在可以跨 CLI、桌面和恢复会话保持可见；Memory 变成项目树式资源面板；侧边栏按项目组织会话并持久保存偏好；插件启停、Skills 和 slash command 列表可以热刷新；同时补上了一批影响日常使用的滚动、复制、附件、缩放、启动诊断和 IM 完成状态问题。

## Highlight

- **/goal 桌面闭环**：新增并打磨 `/goal` 目标循环在桌面端的展示、恢复、历史面包屑和状态同步；恢复会话后仍能看到已完成或进行中的 goal，桌面 composer 保持单一 `/goal` 命令并支持更清晰的 set / clear 交互。
...
- **桌面基础体验打磨**：新增 Settings UI Zoom 滑杆并和快捷键统一；默认纯白主题；支持拖拽附件；修复复制反馈、空会话布局、tab bar 拥挤、长会话滚动、H5 滚动、附件大 payload、数学公式可读性和启动白屏诊断。
- **IM 流式卡片更可靠**：飞书 / 钉钉卡片更新加上完成兜底，避免流式文本已经刷完但卡片仍停在输入中或阻塞 completion。

## 主要更新

- `/goal` 支持从 transcript 恢复 active / completed 状态，并把 goal 生命周期事件作为可见桌面消息恢复出来。
- `/goal` 用户侧语法简化为设置目标和清除目标，避免旧的 status / pause / resume 等管理子命令把目标状态变复杂。
- Goal 活动条和 transcript 事件改用接近 Memory / tool result 的紧凑块状样式，减少大型成功卡片对聊天流的干扰。
- 长时间 goal 运行时，桌面会展示后台 agent 的 start / progress 事件，避免验证或执行阶段看起来像停住了。
...
- 修复旧 `/goal status` 等已移除子命令仍可替换当前目标，导致 goal 状态看起来卡住的问题。
...
- 相关提交已新增和更新 goal state / evaluator、headless command、session restore、后台 agent progress、desktop MessageList、ActiveSession、composerUtils、ChatInput 等测试。
